...
import com.example.init_app.pageview_utils.animation.CoverPageAnim;
import com.example.init_app.pageview_utils.animation.HorizonPageAnim;
import com.example.init_app.pageview_utils.animation.NonePageAnim;
import com.example.init_app.pageview_utils.animation.PageAnimation;
import com.example.init_app.pageview_utils.animation.ScrollPageAnim;
import com.example.init_app.pageview_utils.animation.SimulationPageAnim;
import com.example.init_app.pageview_utils.animation.SlidePageAnim;
...
//        }
    ...
    public PageLoader getPageLoader(String title, String content) {
        Log.e(TAG, "getPageLoader: ");
        // 判是否已经存在
        if (mPageLoader != null) {
            Log.e(TAG, "getPageLoader: 1");
            return mPageLoader;
        }
        Log.e(TAG, "getPageLoader:2");

        // 根据书籍类型，获取具体的加载器
//        if (collBook.isLocal()) {
        mPageLoader = new LocalPageLoader(this, title, content);
...
//        }
        ...
        if (mViewWidth != 0 || mViewHeight != 0) {
            // 初始化 PageLoader 的屏幕大小
//            mPageLoader.setData(title, content);
            Log.e(TAG, "getPageLoader: 3");
            mPageLoader.prepareDisplay(mViewWidth, mViewHeight);
        }
