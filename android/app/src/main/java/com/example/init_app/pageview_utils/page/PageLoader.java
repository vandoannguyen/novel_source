...
import com.example.init_app.utils.RxUtils;
...
//    private BookRecordBean mBookRecord;
    ...
    protected String titlePage = "", contentPage = "";
...
//        mCollBook = collBook;
        ...
        // 初始化数据
        initData(pageView.getContext().getApplicationContext());
        // 初始化画笔
        initPaint();
        // 初始化PageView
        initPageView();
    ...
    public PageLoader(PageView pageView, String titlePage, String contentPage) {
        mPageView = pageView;
        mContext = pageView.getContext();
...
//        mCollBook = collBook;
    ...
    private void initData(Context context) {
        // 获取配置管理器
        mSettingManager = ReadSettingManager.getInstance(context);
        // 获取配置参数
...
//        mTextPaint.set
    ...
    public void setTextSize(int textSize) {
        ...
        // 如果当前已经显示数据
        Log.e(TAG, "setTextSize: isChapterListPrepare" + isChapterListPrepare );
        Log.e(TAG, "setTextSize: mStatus == STATUS_FINISH" + (mStatus == STATUS_FINISH) );
        if (isChapterListPrepare && mStatus == STATUS_FINISH) {
            // 重新计算当前页面
            dealLoadPageList(this.titlePage, this.contentPage);
    ...
    public void setMargin(int marginWidth, int marginHeight) {
        mMarginWidth = ScreenUtils.dpToPx(marginWidth);
        mMarginHeight = ScreenUtils.dpToPx(marginHeight);
...
//        TxtChapter chapter = mChapterList.get(chapterPos);
        ...
        // Xác định xem chương có tồn tại hay không
//        if (!hasChapterData(chapter)) {
//            return null;
//        }
        // Nhận dòng văn bản của chương
        BufferedReader reader = getChapterReader(chapter);
        List<TxtPage> chapters = loadPages(chapter, reader);
        return chapters;
    }

    private List<TxtPage> loadPageList(String title, String data) throws Exception {
        // Nhận chương
//        TxtChapter chapter = mChapterList.get(chapterPos);
        TxtChapter chapter = null;
        // Xác định xem chương có tồn tại hay không
...
//        }
        // Nhận dòng văn bản của chương
//        BufferedReader reader = getChapterReader(chapter);
        List<TxtPage> chapters = loadPages(data, title);
        return chapters;
    }
    ...
    private void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        int tipMarginHeight = ScreenUtils.dpToPx(7);
        if (!isUpdate) {
            /****绘制背景****/
    ...
    private void drawContent(Bitmap bitmap) {
        ...
        if (mStatus != STATUS_FINISH) {
            ...
            switch (mStatus) {
                case STATUS_LOADING:
                    tip = "Đang tải một cách tuyệt vọng ...";
                    break;
                case STATUS_ERROR:
                    tip = "Tải không thành công (nhấp vào cạnh để thử lại)";
                    break;
                case STATUS_EMPTY:
                    tip = "Nội dung bài viết trống";
                    break;
                case STATUS_PARING:
                    tip = "Vui lòng đợi ...";
                    break;
                case STATUS_PARSE_ERROR:
                    tip = "Lỗi phân tích cú pháp tệp";
                    break;
                case STATUS_CATEGORY_EMPTY:
                    tip = "Danh sách thư mục trống";
                    break;
            }
    ...
    void prepareDisplay(int w, int h) {
        ...
        mDisplayWidth = w;
        mDisplayHeight = h;
        Log.e(TAG, "prepareDisplay: " + titlePage.isEmpty() + " " + contentPage.isEmpty());
        // Nhận kích thước của vị trí hiển thị nội dung
...
//        if (!isChapterOpen) {
//            Log.e(TAG, "prepareDisplay: isChapterOpen" );
//            // 展示加载界面
//            mPageView.drawCurPage(false);
...
//            }
//        } else {
//            Log.e(TAG, "prepareDisplay: isChapterOpen else" );
//
//            // 如果章节已显示，那么就重新计算页面
//            if (mStatus == STATUS_FINISH) {
        dealLoadPageList(this.titlePage, this.contentPage);
...
//        }
    ...
    boolean parseNextChapter() {
        ...
        // Lưu vào bộ nhớ đệm danh sách trang của chương hiện tại như chương trước
        mPrePageList = mCurPageList;

        // Dữ liệu chương tiếp theo đã được tải trước hay chưa
        if (mNextPageList != null) {
            ...
            // Xử lý phân tích cú pháp trang
            dealLoadPageList(this.titlePage, this.contentPage);
        }
        // Tải trước trang tiếp theo
        preLoadNextChapter();
        return mCurPageList != null ? true : false;
    }

    private void dealLoadPageList(String title, String data) {
        Log.e(TAG, "dealLoadPageList: " + title);
        try {
            mCurPageList = loadPageList(title, data);
//            cho thêm vào
            mCurPage = mCurPageList.get(0);
...
//            cho thêm vào
    ...
    private void preLoadNextChapter() {
        ...
        // Nếu chương tiếp theo không tồn tại và không có dữ liệu trong chương tiếp theo, nó sẽ không được tải.
        if (!hasNextChapter()
                || !hasChapterData(mChapterList.get(nextChapter))) {
            return;
        }

        //Hủy nếu nó đã được tải trước đó
        if (mPreLoadDisp != null) {
            mPreLoadDisp.dispose();
        }

        //Gọi không đồng bộ để tải trước
        Single.create(new SingleOnSubscribe<List<TxtPage>>() {
            @Override
            public void subscribe(SingleEmitter<List<TxtPage>> e) throws Exception {
                e.onSuccess(loadPageList(titlePage, contentPage));
            }
    ...
    private List<TxtPage> loadPages(TxtChapter chapter, BufferedReader br) {
        ...
        // Tải bằng cách sử dụng tính năng phát trực tuyến
        List<String> lines = new ArrayList<>();
        int rHeight = mVisibleHeight;
        int titleLinesCount = 0;
        boolean showTitle = false; // Có hiển thị tiêu đề không
        String paragraph = "title";// Tiêu đề hiển thị mặc định
        String data = "", byteData = "";
        List<String> listParagraph = new ArrayList<>();
        try {
...
//            listParagraph.add(paragraph);
            listParagraph.addAll(Arrays.asList(data.split("[\\n]")));
            while (showTitle || !listParagraph.isEmpty()) {
                paragraph = listParagraph.get(0);
                // Đặt lại đoạn văn
                if (!showTitle) {
                    if (paragraph.equals("")) {
                        listParagraph.remove(0);
                        continue;
                    }
                    paragraph = "   " + paragraph + "\n";
                ...
                int wordCount = 0;
                StringBuilder subStrBuilder = null;
                while (paragraph.length() > 0) {
                    //Không gian hiện tại có thể chứa dòng văn bản tiếp theo hay không
                    if (showTitle) {
                        rHeight -= mTitlePaint.getTextSize();
                    } else {
                        rHeight -= mTextPaint.getTextSize();
                    }
                    //Một trang được lấp đầy, hãy tạo TextPage
                    if (rHeight <= 0) {
                        // 创建Page Tạo trang
                        TxtPage page = new TxtPage();
                        page.position = pages.size();
                        page.title = "title";//StringUtils.convertCC(chapter.getTitle(), mContext);
                    ...
                    //测量一行占用的字节数 Đo số byte được chiếm bởi một hàng
                    if (showTitle) {
                    ...
                    for (int i = 0; i < splitString.length; i++) {
                        if (subStrBuilder.length() + 1 + splitString[i].length() > wordCount) {
                            break;
                        }
                        subStrBuilder.append(splitString[i] + " ");
//                        if(splitString[i].contains("\n")) break;
                ...
                //增加段落的间距 Tăng khoảng cách đoạn văn
            ...
            if (lines.size() != 0) {
                ...
                TxtPage page = new TxtPage();
                page.position = pages.size();
                page.title = "title";// StringUtils.convertCC(title, mContext);
                page.lines = new ArrayList<>(lines);
                page.titleLines = titleLinesCount;
                pages.add(page);
                //重置Lines Đặt lại dòng
                lines.clear();
            }
        } catch (FileNotFoundException e) {
            Log.e(TAG, "loadPages: 1" + e);
            e.printStackTrace();
        } catch (IOException e) {
            Log.e(TAG, "loadPages: 2" + e);
            e.printStackTrace();
        } catch (Exception e) {
            Log.e(TAG, "loadPages: 3" + e);
        }
        return pages;
    }

    private List<TxtPage> loadPages(String content, String title) {
        // Trang đã tạo
        Log.e(TAG, "loadPages: " + content);
        Log.e(TAG, "loadPages: " + title);
        List<TxtPage> pages = new ArrayList<>();
        // Tải bằng cách sử dụng tính năng phát trực tuyến
        List<String> lines = new ArrayList<>();
        int rHeight = mVisibleHeight;
        int titleLinesCount = 0;
        boolean showTitle = false; // Có hiển thị tiêu đề không
        String paragraph = "";// Tiêu đề hiển thị mặc định
        String data = "";
        List<String> listParagraph = new ArrayList<>();
        try {
            data = content;
...
//                        if(splitString[i].contains("\n")) break;
    ...
    /**
     * @return:Lấy trang được hiển thị ban đầu
     */
    ...
    /**
     * @return:Lấy trang cuối cùng của chương trước
     */
    ...
    /**
     * Theo tình trạng hiện tại, hãy quyết định xem có lật trang hay không
     *
     * @return
     */
